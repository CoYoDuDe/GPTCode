...
# gptcode.py schreiben
backup_file "$APP_DIR/gptcode.py"
cat > "$APP_DIR/gptcode.py" <<'PY'
#!/usr/bin/env python3
"""
...
# Wrapper
backup_file "$BIN_DIR/gptcode"
cat > "$BIN_DIR/gptcode" <<'SH'
#!/usr/bin/env bash
APP_DIR="/opt/gptcode"
PY="$APP_DIR/gptcode.py"
VENV="$APP_DIR/venv"

source "$VENV/bin/activate"
PYTHONWARNINGS=ignore::DeprecationWarning python3 "$PY" "$@"
exit_code=$?
deactivate
exit $exit_code
SH
chmod 0755 "$BIN_DIR/gptcode"

echo -e "\n✅ Installation abgeschlossen. Nutze ab jetzt:  gptcode\n"
